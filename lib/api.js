...
async function req(path, options = {}) {
  ...
  if (!res.ok) {
    // Se o servidor mandou um erro estruturado ({ "error": "..." }), mostra a
    // mensagem dele limpa — é assim que o aviso simpático do limite diário
    // chega à tela, em vez de um JSON cru com código de status.
...
export const api = {
  ...
  // tema do canal (a IA inteira se molda a ele)
  niche: () => req("/niche"),
  setNiche: (niche) => req("/niche", { method: "PUT", body: JSON.stringify({ niche }) }),
